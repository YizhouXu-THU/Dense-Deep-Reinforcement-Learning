...
simulation_config["gui_flag"] = True
simulation_config["step_size"] = 0.1
simulation_config["action_step_size"] = 0.1
simulation_config["max_step_num"] = 10000
# renkun 0818
# simulation_config["speed_range"] = [5, 25]
# v_resolution=1时，速度上下限范围的差值应该为20，满足(v_max - v_min) / v_resolution = 20
# xyz 0922
simulation_config["speed_range"] = [0, 40]
...
experiment_config["traj_data_save_path"] = "./trajs"
...
gen_length = 500 # the traffic flow will not be generated after this length alongside the road from the begining point  # xyz 0922
...
# xyz 0922
def update_road_info(road_max_speed):
    simulation_config["speed_range"] = [0, road_max_speed]
    
    # =========== NDD distribution for car following / free flow ===========
    
    # renkun 0818
    # v_low, v_high, r_low, r_high, rr_low, rr_high, acc_low, acc_high = 15, 35, 35, 150, -10, 8, -4, 2
    r_low, r_high, rr_low, rr_high, acc_low, acc_high = 35, 150, -10, 8, -4, 2
    v_low, v_high = simulation_config["speed_range"]

    # v_resolution, r_resolution, rr_resolution, acc_resolution = 1, 1, 1, 0.2
    r_resolution, rr_resolution, acc_resolution = 1, 1, 0.2
    v_resolution = (v_high - v_low) / 20.0
    # if print_log_flag:
    ...
    # renkun 0818
    one_lead_r_low, one_lead_r_high, one_lead_rr_low, one_lead_rr_high = 35, 150, -10, 8
    one_lead_v_low, one_lead_v_high = simulation_config["speed_range"]
    # one_lead_r_low, one_lead_r_high, one_lead_rr_low, one_lead_rr_high, one_lead_v_low, one_lead_v_high = 0, 115, -10, 8, 20, 40
    one_lead_r_low, one_lead_r_high, one_lead_rr_low, one_lead_rr_high = 0, 115, -10, 8
    one_lead_r_step, one_lead_rr_step, one_lead_v_step = 1, 1, v_resolution    # xyz 0927
    ...
    one_lead_speed_list, one_lead_r_list, one_lead_rr_list = list(np.arange(one_lead_v_low, one_lead_v_high+one_lead_v_step, one_lead_v_step)), list(range(one_lead_r_low, one_lead_r_high+one_lead_r_step, one_lead_r_step)), list(range(one_lead_rr_low, one_lead_rr_high+one_lead_rr_step,one_lead_rr_step))
    ...
    for i in range(int((one_lead_v_high-one_lead_v_low+one_lead_v_step)/one_lead_v_step)): one_lead_v_to_idx_dic[list(np.arange(one_lead_v_low, one_lead_v_high+one_lead_v_step, one_lead_v_step))[i]] = i
    ...
    # renkun 0818
    lc_v_num, lc_v_to_idx_dic = 21, bidict()
    lc_v_low, lc_v_high = simulation_config["speed_range"]
    ...
    SM_IDM_DESIRED_VELOCITY = v_high * 0.8
...
update_road_info(road_max_speed=simulation_config["speed_range"][1])
...
LANE_CHANGE_SCALING_FACTOR = 1.0
...
OL_LC_low_speed_flag, OL_LC_low_speed_use_v = False, 24
