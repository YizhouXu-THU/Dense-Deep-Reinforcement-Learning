...
# TODO: predict时车辆行驶跨越lane的情况：延申最后一个segment
# TODO: junction处：忽略，不计算criticality
...
def get_circle_center_list(traj_point):
    ...
    center1 = (traj_point["x_global"], traj_point["y_global"])
...
class Traj:
    ...
    def __init__(self, x_global, y_global, heading, x_lon, x_lat, v_lon, v_lat, lane_index, lane_list_info, road_id):
        self.traj_info = {}
        self.lane_list_info = np.array(lane_list_info)
        self.road_id = road_id  # renkun 0819

        # renkun 0819
        # self.traj_info["0.0"] = {"x_lon": x_lon, "x_lat": x_lat,
        #                          "v_lon": v_lon, "v_lat": v_lat, "lane_index": lane_index}
        self.traj_info["0.0"] = {"x_global": x_global, "y_global": y_global, "heading": heading * math.pi / 180,
                                 "x_lon": x_lon, "x_lat": x_lat,
                                 "v_lon": v_lon, "v_lat": v_lat,
    ...
    def predict_with_action(self, action, start_time=0.0, time_duration=1.0):
        ...
        #     predict_traj_info["lane_index"] = ini_traj_info["lane_index"] - 1
        #     predict_traj_info["x_lat"] = ini_traj_info["x_lat"] - 4.0
        if action["lateral"] == "left" and ini_traj_info["lane_index"] + 1 < len(self.lane_list_info):
            predict_traj_info["lane_index"] = ini_traj_info["lane_index"] + 1
            predict_traj_info["x_lat"] = ini_traj_info["x_lat"] + (self.lane_list_info[ini_traj_info["lane_index"] + 1] - self.lane_list_info[ini_traj_info["lane_index"]])
        elif action["lateral"] == "right" and ini_traj_info["lane_index"] - 1 >= 0:
            predict_traj_info["lane_index"] = ini_traj_info["lane_index"] - 1
            predict_traj_info["x_lat"] = ini_traj_info["x_lat"] - (self.lane_list_info[ini_traj_info["lane_index"]] - self.lane_list_info[ini_traj_info["lane_index"] - 1])
        else:
        ...
        # renkun 0819
        global_x, global_y, heading = self.calculate_global_position(self.road_id,
                                                                     predict_traj_info["x_lon"],
                                                                     predict_traj_info["x_lat"],
                                                                     predict_traj_info["v_lon"],
                                                                     predict_traj_info["v_lat"],
                                                                     np.max(self.lane_list_info))
        predict_traj_info["x_global"] = global_x
        predict_traj_info["y_global"] = global_y
        predict_traj_info["heading"] = heading
        ...
        if time_duration == 1.0:
            ...
            if is_lane_change is None:
                if ini_traj_info["v_lat"] > 0.5 and not ini_traj_info["x_lat"] >= max(self.lane_list_info):
                    predict_traj_info["lane_index"] = np.argwhere(self.lane_list_info > ini_traj_info["x_lat"])[0].item()
                # renkun 0819
                # elif ini_traj_info["v_lat"] < -0.5 and not ini_traj_info["x_lat"] <= min(TreeSearchNADEBackgroundController.lane_list):
                elif ini_traj_info["v_lat"] < -0.5 and not ini_traj_info["x_lat"] <= min(self.lane_list_info):
                    predict_traj_info["lane_index"] = np.argwhere(self.lane_list_info < ini_traj_info["x_lat"])[-1].item()
                else:
                    predict_traj_info["lane_index"] = ini_traj_info["lane_index"]
            else:
                ...
                elif is_lane_change and ini_traj_info["v_lat"] < -0.5 and not ini_traj_info["x_lat"] <= min(self.lane_list_info):
                    predict_traj_info["lane_index"] = np.argwhere(self.lane_list_info < ini_traj_info["x_lat"])[-1].item()
                else:
                    predict_traj_info["lane_index"] = ini_traj_info["lane_index"]
            new_x_lat = self.lane_list_info[int(predict_traj_info["lane_index"])]
            ...
            if not is_lane_change or estimated_lane_change_time > 1: # only activate when the vehicle is not lane changing
                predict_traj_info["x_lat"] = self.lane_list_info[int(predict_traj_info["lane_index"])]
                if action is None:
                ...
                # renkun 0819
                global_x, global_y, heading = self.calculate_global_position(self.road_id, predict_traj_info["x_lon"],
                                                                             predict_traj_info["x_lat"],
                                                                             predict_traj_info["v_lon"],
                                                                             predict_traj_info["v_lat"],
                                                                             np.max(self.lane_list_info))
                ...
                predict_traj_info["x_lat"] = self.lane_list_info[int(predict_traj_info["lane_index"])]
                predict_traj_info["x_lon"] = ini_traj_info["x_lon"] + ini_traj_info["v_lon"]*estimated_lane_change_time
                predict_traj_info["v_lat"] = 0
                predict_traj_info["v_lon"] = ini_traj_info["v_lon"]
                ...
                self.traj_info["%.1f" % (start_time + estimated_lane_change_time)] = predict_traj_info
                self.predict_without_action(
                    start_time + estimated_lane_change_time, 1-estimated_lane_change_time)
                self.interpolate(start_time=start_time, end_time=start_time+estimated_lane_change_time)
                self.interpolate(start_time=start_time+estimated_lane_change_time, end_time=start_time + time_duration)
        else:
        ...
        if ("%.1f" % start_time) in self.traj_info and ("%.1f" % end_time) in self.traj_info:
            ...
            for time_value in inter_list:
                if isclose(time_value, start_time) or isclose(time_value, end_time):
                    ...
                    start_ratio = (end_time - time_value) / (end_time - start_time)
                    tmp_traj = {}
                    tmp_traj["x_lon"] = start_ratio * ini_info["x_lon"] + (1-start_ratio) * end_info["x_lon"]
                    tmp_traj["x_lat"] = start_ratio * ini_info["x_lat"] + (1-start_ratio) * end_info["x_lat"]
                    tmp_traj["v_lon"] = start_ratio * ini_info["v_lon"] + (1-start_ratio) * end_info["v_lon"]
                    tmp_traj["v_lat"] = start_ratio * ini_info["v_lat"] + (1-start_ratio) * end_info["v_lat"]
                    tmp_traj["lane_index"] = (np.abs(self.lane_list_info - tmp_traj["x_lat"])).argmin().item()
                    ...
                    global_x, global_y, heading = self.calculate_global_position(self.road_id,
                                                                                 ...
                                                                                 tmp_traj["v_lat"],
                                                                                 np.max(self.lane_list_info))
                    tmp_traj["x_global"] = global_x
                    tmp_traj["y_global"] = global_y
                    tmp_traj["heading"] = heading
    ...
    # renkun 0819
    def calculate_global_position(self, edge_id, edge_position, lateral_position, v_lon, v_lat, edge_width):
        cumulative_length = 0.0

        edge = conf.sumo_net.getEdge(edge_id)
        edge_shape = edge.getShape()
        lateral_position = lateral_position - edge_width / 2

        i = 0
        while True:
        # for i in range(len(edge_shape) - 1):
            if i >= len(edge_shape) - 1:
                edge_shape.append((edge_shape[-1][0] + edge_shape[-1][0] - edge_shape[-2][0], edge_shape[-1][1] + edge_shape[-1][1] - edge_shape[-2][1]))
            x1, y1 = edge_shape[i]
            x2, y2 = edge_shape[i + 1]
            segment_length = math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)

            if cumulative_length + segment_length >= edge_position:
                segment_fraction = (edge_position - cumulative_length) / segment_length
                base_x = x1 + segment_fraction * (x2 - x1)
                base_y = y1 + segment_fraction * (y2 - y1)

                dx = x2 - x1
                dy = y2 - y1
                norm = math.sqrt(dx * dx + dy * dy)
                lateral_x = -dy / norm * lateral_position
                lateral_y = dx / norm * lateral_position

                global_x = base_x + lateral_x
                global_y = base_y + lateral_y

                heading_lane = math.atan2(v_lat, v_lon)
                heading = math.atan2(dy, dx) - math.pi / 2 + heading_lane

                return global_x, global_y, heading

            cumulative_length += segment_length
            i += 1

        # 如果位置超出了线段的范围，返回最后一个点（应该不太可能发生）
        # TODO:
...
class TreeSearchNADEBackgroundController(NADEBackgroundController):
    ...
    def update_single_vehicle_obs_no_action(veh, duration=1.0, is_lane_change=None, action=None):
        ...
        initial_traj = Traj(veh["position"][0], veh["position"][1], veh["heading"],
                            veh["lane_position"], veh["lateral_offset"],
                            veh["velocity"], veh["lateral_speed"],
                            veh["lane_index"], veh["lane_list_info"], veh["road_id"])
        if is_lane_change is None:
            is_lane_change = utils.is_lane_change(veh)
        new_traj_result = initial_traj.predict_without_action(0.0, duration, is_lane_change, action)
        # new_veh = ujson.loads(ujson.dumps(veh))
        new_veh = dict(veh)

        # renkun 0819
        # new_veh["position"] = (new_traj_result["x_lon"], new_traj_result["x_lat"])
        new_veh["position"] = (new_traj_result["x_global"], new_traj_result["y_global"])
        ...
        if new_veh["lane_index"] == len(initial_traj.lane_list_info) - 1:
    ...
    def is_CF(cav_obs, bv_obs):
        ...
        if bv_r1 is not None and bv_r1['veh_id'] == CAV_id:
            ...
            bv_v = bv_obs["Ego"]["velocity"]
            bv_range_CAV = bv_obs["Ego"]["position"][0] - cav_obs["Ego"]["position"][0] - conf.LENGTH
            bv_rangerate_CAV = bv_obs["Ego"]["velocity"] - cav_obs["Ego"]["velocity"]
        if bv_f1 is not None and bv_f1['veh_id'] == CAV_id:
            ...
            bv_rangerate_CAV = cav_obs["Ego"]["velocity"] - bv_obs["Ego"]["velocity"]
    ...
    def update_single_vehicle_obs(veh, action, duration=conf.simulation_resolution):
        ...
        if (action == "left" or action == "right") and not utils.is_lane_change(veh):
            ...
            #             veh["velocity"], veh["lateral_speed"], veh["lane_index"])
            traj = Traj(veh["position"][0], veh["position"][1], veh["heading"],
                        veh["lane_position"], veh["lateral_offset"],
                        veh["velocity"], veh["lateral_speed"],
                        veh["lane_index"], veh["lane_list_info"], veh["road_id"])
            ...
            # renkun 0819
            # if new_veh["lane_index"] == len(conf.lane_list) -1:
            if new_veh["lane_index"] == len(traj.lane_list_info) - 1:
                new_veh["could_drive_adjacent_lane_left"] = False
                new_veh["could_drive_adjacent_lane_right"] = True
        else:
            new_veh, traj = TreeSearchNADEBackgroundController.update_single_vehicle_obs_no_action(veh, duration, action)
        return new_veh, traj
    ...
    def traj_to_obs(prev_full_obs, full_traj, time):
        ...
        for veh_id in full_traj:
            ...
            obs[veh_id]["position"] = (full_traj[veh_id].traj_info[time]["x_global"], full_traj[veh_id].traj_info[time]["y_global"])
    ...
    def update_obs(full_obs, cav_id, bv_id, bv_action, cav_action, predicted_full_obs=None, predicted_full_traj=None):
        ...
        for veh_id in full_obs:
            ...
            if action:
                ...
                if predicted_full_obs is None or predicted_full_traj is None:
                    new_full_obs[veh_id], trajectory_obs[veh_id] = TreeSearchNADEBackgroundController.update_single_vehicle_obs(vehicle, action)
                else:
                    new_full_obs[veh_id] = predicted_full_obs[veh_id][action]
                    trajectory_obs[veh_id] = predicted_full_traj[veh_id][action]
        new_full_obs[cav_id], trajectory_obs[cav_id] = TreeSearchNADEBackgroundController.update_single_vehicle_obs(new_full_obs[cav_id], cav_action)
        ...
        for veh_id in new_full_obs:
            ...
            new_full_obs[veh_id]["euler_distance"] = utils.cal_euclidean_dist(av_pos, bv_pos)
        new_full_obs = OrderedDict(sorted(new_full_obs.items(), key=lambda item: item[1]['euler_distance']))
    ...
    def full_obs_to_cav_bv_obs(full_obs, cav_id, bv_id):
        ...
        for key in full_obs:
            new_full_obs[key] = dict(full_obs[key])
        cav_obs = TreeSearchNADEBackgroundController.full_obs_to_single_obs(new_full_obs, cav_id)
        bv_obs = TreeSearchNADEBackgroundController.full_obs_to_single_obs(new_full_obs, bv_id)
    ...
    def leaf_node_check(full_obs, previous_obs, traj, cav_obs, bv_obs, cav_id, bv_id, search_depth):
        ...
        depth_flag = (search_depth == TreeSearchNADEBackgroundController.MAX_TREE_SEARCH_DEPTH)
        CF_flag = TreeSearchNADEBackgroundController.is_CF(cav_obs, bv_obs)[0]
        bv_id = bv_obs["Ego"]["veh_id"]
        if CF_flag:
            challenge_array[2:] = TreeSearchNADEBackgroundController.get_CF_challenge_array(cav_obs, bv_obs)
        crash_flag = TreeSearchNADEBackgroundController.crash_check(cav_obs, bv_obs, cav_id, bv_id, previous_obs, traj)
        if crash_flag:
            challenge_array = np.ones(TreeSearchNADEBackgroundController.ACTION_NUM)
    ...
    def tree_search_maneuver_challenge(full_obs, previous_obs, traj, cav_id, bv_id, search_depth, cav_obs=None, bv_obs=None, predicted_full_obs=None, predicted_full_traj=None):
        ...
        if (not cav_obs) or (not bv_obs):
            cav_obs, bv_obs = TreeSearchNADEBackgroundController.full_obs_to_cav_bv_obs(full_obs, cav_id, bv_id)
        leaf_flag, leaf_challenge_array = TreeSearchNADEBackgroundController.leaf_node_check(full_obs, previous_obs, traj, cav_obs, bv_obs, cav_id, bv_id, search_depth)
        cav_action_dict, bv_action_dict, cav_pdf, bv_pdf = TreeSearchNADEBackgroundController.get_cav_bv_pdf(cav_obs, bv_obs)
    ...
    def get_CF_challenge_array(cav_obs, bv_obs):
        CF_info, bv_v, bv_range_CAV, bv_rangerate_CAV = TreeSearchNADEBackgroundController.is_CF(cav_obs, bv_obs)
        if not CF_info:
    ...
    def get_cav_bv_pdf(cav_obs, bv_obs):
        cav_pdf = TreeSearchNADEBackgroundController.SURROGATE_MODEL_FUNCTION(cav_obs)
        ...
        if utils.is_lane_change(cav_obs["Ego"]):
            ...
            cav_action_dict = {"left": cav_pdf[0], "right": cav_pdf[2], "still": cav_pdf[1]}
        if utils.is_lane_change(bv_obs["Ego"]):
            ...
            bv_action_dict = {"left": bv_pdf[0], "right": bv_pdf[1], "still": np.sum(bv_pdf[2:])}
